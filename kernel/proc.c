...
void exit(int status)
{
  ...
  p->endtime = ticks;
    ...
    pop(qno, p);
  #endif
  //printf("Creation time: %d End time: %d\n", p->ctime, p->endtime);
  //printf("Sleep time: %d Run time: %d\n",p->sleeptime,p->runtime);
  release(&wait_lock);
    ...
    for (np = proc; np < &proc[NPROC]; np++)
    {
      if (np->parent == p)
      {
        ...
        if (np->state == ZOMBIE)
        {
          ...
          np->endtime = ticks;
          //printf("pid: %d start time: %d run time: %d end time: %d %d\n",np->pid,np->ctime,np->runtime,np->endtime,np->sleeptime);
          freeproc(np);
          release(&np->lock);
          release(&wait_lock);
          return pid;
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      if (p->state != RUNNABLE)
        continue;
      acquire(&p->lock);
      int dp = calculatedp(p);
      p->dynamicpriority = dp;
      if (q == 0)
        q = p;
      if (q->dynamicpriority < p->dynamicpriority)
      {
        q = p;
      }
      else if (q->dynamicpriority == p->dynamicpriority && q->schedcount < p->schedcount)
      {
        q = p;
      }
      else if (q->dynamicpriority == p->dynamicpriority && q->schedcount == p->schedcount && q->ctime < p->ctime)
      {
        q = p;
      }
      release(&p->lock);
    ...
    if (q == 0)
      continue;
    //printf("Niceness: %d Dynamic Priority: %d\n", q->niceness, q->dynamicpriority);
    acquire(&q->lock);
    if (q->state == RUNNABLE)
    {
      //printf("Niceness: %d Dynamic Priority: %d\n", q->niceness, q->dynamicpriority);
      //printf("Sleep time: %d Run time: %d\n",q->sleeptime,q->runtime);
      q->lastruntime = 0;
      q->lastsleeptime = 0;
      q->sleepstart = 0;
      q->state = RUNNING;
